...
public class DeviceReducer extends Reducer<Text, Text, Text, Text> {
    ...
    public void reduce(Text key, Iterable<Text> values, Context context) 
        ...
        for (Text value : values) {
            ...
            if (parts.length == 4) {
                ...
                totalBidValue += Double.parseDouble(parts[2]);
        ...
        double avgBidPrice = (totalImpressions > 0) ? 
            (totalBidValue / totalImpressions) : 0;
        double avgRevenue = (totalImpressions > 0) ? 
            (totalRevenue / totalImpressions) : 0;
        double revenuePerClick = (totalClicks > 0) ? 
            (totalRevenue / totalClicks) : 0;
        
        String output = String.format("%d,%d,%d,%.4f,%.2f,%.2f,%.4f,%.4f,%.4f", 
            totalImpressions,
            totalClicks,
            totalConversions,
            totalRevenue,
